...
def decode_unknown_signals(decoded_signals, unknown_signals):
...
def solution2(notes):
    ...
    for unique_signals, output_signals in notes:
        ...
        decoded_signals = decode_unknown_signals(decoded_signals, unknown_signals)
        digits.append(
            "".join(
                [
...
input = open(os.getcwd() + "\Day_08\input.txt").read().splitlines()
get_notes = lambda notes: [list(map(set, signal.split())) for signal in notes]
signal_notes = [get_notes(note.split("|")) for note in input]

print(solution1(signal_notes))
print(solution2(signal_notes))